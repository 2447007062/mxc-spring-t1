...
layui.define(['laytpl', 'layer'], function(exports){
  ...
  view.exit = function(callback){
	...
	layui.data('memberInfo', {key: 'value',remove: true});
  ...
  view.req = function(options){
    ...
    options.headers = options.headers || {};
      ...
      /**
       *  @describe 自动给header加入与后台约定的参数
       *  @author javaGuo
       *  @date 20190222
       */
    if(request.tokenName && layui.data('memberInfo') && layui.data('memberInfo').value){
        var privateKey = "springbootsystem";
        var publicKey = "" + new Date().getTime();
        var sign = hex_md5((privateKey + publicKey));
        options.headers[request.tokenName] = layui.data('memberInfo').value.token;
        options.headers[request.publicKey] = publicKey;
        options.headers[request.sign] = sign;
    }
    ...
    return $.ajax($.extend({
      ...
      ,success: function(res){
        ...
        if(res.code == "0000001") {
    		...
    		var exp = new Date();
        	layui.data('memberInfo', {key: 'info',value: res.data.info});
        	layui.data('memberInfo', {key: 'expires',value: exp.getTime() + seconds});
        	typeof options.done === 'function' && options.done(res);
        }
        //执行成功
        else if(res.code == "0000000") typeof options.done === 'function' && options.done(res);
        //令牌失效或不存在，清除本地 access_token，并强制跳转到登入页
        else if(res.code == "0000005"){
        	view.exit();
        }
        //其它异常
        else {
          var error = ['<cite>Error：</cite> ' + (res.msg || '系统异常')].join('');
          view.error(error);
        }
