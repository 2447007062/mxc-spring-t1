...
import top.javaguo.utils.GuoAddressUtil;
...
public class SSOController {
	 ...
	 */
	@PostMapping("/login")
	public RespBean<Map<String, Object>> login(String username, String password, HttpServletRequest request)
			throws Exception {
		...
		if (GuoStringUtil.isEmpty(username) || GuoStringUtil.isEmpty(password))
			...
			try {
				...
				if (sysUser != null) {
					...
					// 封装返回给前台的数据
					SysUserResult sysUserResult = new SysUserResult(sysUser.getId(), sysUser.getUsername(), token,
                    ...
                    guoRedisUtil.set(token, GuoJsonUtil.Object2Json(sysUserResult), 1800);
                    // 根据ip获取用户所在地址并修改
